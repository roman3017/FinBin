...
-/
def has_linear_representation {n : ℕ} (f : ZMod n → ZMod n) : Prop :=
...
-/
def in_cycle {n : ℕ} (f : ZMod n → ZMod n) (x : ZMod n) : Prop :=
  ∃ k > 0, f^[k] x = x
...
def components {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) : Finset (ZMod n) :=
  Finset.univ.image (component_rep f)

/-
The cycle position is the number of steps from the component representative to the root of x.
-/
noncomputable def cycle_position {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) : ℕ :=
  Nat.find (show ∃ k, f^[k] (component_rep f x) = root f x by
              -- By definition of component_rep, we know that component_rep f x is in the cycle of x.
              have h_cycle : component_rep f x ∈ cycle_elements f x := by
                have h_root_in_cycle : ∃ y ∈ Finset.image ZMod.val (cycle_elements f x), ∀ z ∈ Finset.image ZMod.val (cycle_elements f x), y ≤ z := by
                  apply_rules [ Finset.exists_min_image ];
                  simp +zetaDelta at *;
                  exact ⟨ _, Finset.mem_image_of_mem _ ( Finset.mem_range.mpr ( cycle_period_pos f x ) ) ⟩;
                unfold component_rep;
                obtain ⟨ y, hy₁, hy₂ ⟩ := h_root_in_cycle;
                have h_min_val : (Finset.image ZMod.val (cycle_elements f x)).min' (by
                exact ⟨ _, hy₁ ⟩) = y := by
              ...
              have h_cycle_period : f^[cycle_period f x] (root f x) = root f x := by
                exact iterate_cycle_period_root f x;
              use cycle_period f x - k;
...
-/
noncomputable def max_depth {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) : ℕ :=
  Finset.univ.sup (fun x => depth f x)

/-
If x is in a cycle, then the root of x is x itself.
-/
lemma root_eq_self_of_in_cycle {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) (h : in_cycle f x) :
  root f x = x := by
...
-/
lemma in_cycle_of_mem_cycle_elements {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x y : ZMod n) (h : y ∈ cycle_elements f x) :
  in_cycle f y := by
  -- By definition of cycle elements, there exists some k such that f^[k] (root f x) = y.
  obtain ⟨k, hk⟩ : ∃ k, f^[k] (root f x) = y := by
    unfold cycle_elements at h; aesop;
  -- By definition of $in_cycle$, we need to show that there exists some $m > 0$ such that $f^m(y) = y$.
  use cycle_period f x;
  rw [ ← hk ];
...
-/
lemma cycle_period_eq_of_mem_cycle_elements {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x y : ZMod n) (h : y ∈ cycle_elements f x) :
  cycle_period f x = cycle_period f y := by
  ...
  obtain ⟨m, hm⟩ : ∃ m, f^[m] y = root f x := by
    ...
    use cycle_period f x - k % cycle_period f x;
    rw [ hk, ← Nat.mod_add_div k ( cycle_period f x ) ] ; simp +decide [ Function.iterate_add_apply, Function.iterate_mul ] ;
    simp +decide [ ← Function.iterate_add_apply, Nat.sub_add_cancel ( Nat.mod_lt _ ( cycle_period_pos f x ) |> Nat.le_of_lt ) ];
    induction k / cycle_period f x <;> simp_all +decide [ Function.iterate_succ_apply' ];
    · exact iterate_cycle_period_root f x;
    · exact iterate_cycle_period_root f x;
  -- Since $y$ is in the cycle elements of $x$, we have $f^[cycle\_period f x] y = y$.
  have h_cycle_period_y : f^[cycle_period f x] y = y := by
    have h_cycle_period_y : f^[cycle_period f x] (root f x) = root f x := by
      exact iterate_cycle_period_root f x;
    simp_all +decide [ ← Function.iterate_add_apply ];
    rw [ add_comm, Function.iterate_add_apply, h_cycle_period_y ];
  have h_cycle_period_y : cycle_period f y ≤ cycle_period f x := by
  ...
  have h_cycle_period_x : f^[cycle_period f y] (root f x) = root f x := by
...
-/
lemma cycle_period_apply_of_in_cycle {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) (h : in_cycle f x) :
  cycle_period f (f x) = cycle_period f x := by
  ...
  have h_cycle_elements : f x ∈ cycle_elements f x := by
    unfold cycle_elements;
    have h_root : root f x = x := by
      exact root_eq_self_of_in_cycle f x h;
    rcases k : cycle_period f x with ( _ | _ | k ) <;> simp_all +decide;
...
lemma component_rep_mem_cycle {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) :
  component_rep f x ∈ cycle_elements f x := by
  unfold component_rep cycle_elements;
  -- By definition of `Finset.min'`, the minimum element in the image of the cycle elements is indeed in the image.
  have h_min_in_image : (Finset.image ZMod.val (Finset.image (fun k => f^[k] (root f x)) (Finset.range (cycle_period f x)))).min' (by
  exact ⟨ _, Finset.mem_image_of_mem _ ( Finset.mem_image_of_mem _ ( Finset.mem_range.mpr ( cycle_period_pos f x ) ) ) ⟩) ∈ Finset.image ZMod.val (Finset.image (fun k => f^[k] (root f x)) (Finset.range (cycle_period f x))) := by
...
-/
lemma cycle_period_eq_of_component_rep_eq {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x y : ZMod n) (h : component_rep f x = component_rep f y) :
  cycle_period f x = cycle_period f y := by
  -- Since the component representatives are the same, the smallest k such that f^k(component_rep f x) = component_rep f x must be the same as the smallest k such that f^k(component_rep f y) = component_rep f y.
  have h_cycle_period_eq : cycle_period f (component_rep f x) = cycle_period f (component_rep f y) := by
    rw [h];
  have h_cycle_period_eq : cycle_period f x = cycle_period f (component_rep f x) := by
    apply cycle_period_eq_of_mem_cycle_elements;
    exact component_rep_mem_cycle f x
  have h_cycle_period_eq' : cycle_period f y = cycle_period f (component_rep f y) := by
    apply cycle_period_eq_of_mem_cycle_elements;
...
def jp {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (p : ℕ) (zeta : ZMod p) (x : ZMod n) : ZMod p :=
  zeta ^ (cycle_position f (root f x)) * (zeta⁻¹) ^ (depth f x)
...
def signature_map {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) : ℕ :=
  (preimages f (f x)).idxOf x

/-
Injectivity of the signature map for elements with the same image.
-/
lemma signature_inj {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x y : ZMod n) (h_eq : f x = f y) (h_sig : signature_map f x = signature_map f y) : x = y := by
  unfold signature_map at h_sig;
...
-/
lemma signature_bound {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) : signature_map f x < n := by
...
-/
lemma depth_f_of_not_in_cycle {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) (h : ¬ in_cycle f x) :
  depth f (f x) = depth f x - 1 := by
    ...
    have h_depth_def : in_cycle f (f^[k] x) ∧ ∀ j < k, ¬in_cycle f (f^[j] x) := by
...
-/
lemma root_f_eq_root_of_not_in_cycle {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) (h : ¬ in_cycle f x) :
...
lemma iterate_eq_iff_modEq_of_component_rep {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) (k j : ℕ) :
  f^[k] (component_rep f x) = f^[j] (component_rep f x) ↔ k ≡ j [MOD cycle_period f x] := by
    -- Let `rep = component_rep f x`.
    set rep := component_rep f x;
    -- Since `rep` is in the cycle of `x`, `cycle_period f rep = cycle_period f x`.
    have h_cycle_period_rep : cycle_period f rep = cycle_period f x := by
      have h_cycle_period_rep : rep ∈ cycle_elements f x := by
        exact component_rep_mem_cycle f x;
      exact Eq.symm (cycle_period_eq_of_mem_cycle_elements f x rep h_cycle_period_rep);
    -- Since `rep` is in the cycle of `x`, `f^[p] rep = rep` where `p = cycle_period f rep`.
    have h_cycle_rep : f^[cycle_period f rep] rep = rep := by
    ...
    have h_distinct : ∀ k l : ℕ, k < l → l < cycle_period f rep → f^[k] rep ≠ f^[l] rep := by
      intros k l hkl hl_lt_cycle_period h_eq
      have h_contradiction : f^[cycle_period f rep - l + k] rep = rep := by
        have h_contradiction : f^[cycle_period f rep - l] (f^[l] rep) = rep := by
      ...
      have h_contradiction : cycle_period f rep ≤ cycle_period f rep - l + k := by
        apply Nat.find_min';
        have h_root_eq_rep : root f rep = rep := by
          apply root_eq_self_of_in_cycle;
          exact ⟨ cycle_period f rep, Nat.pos_of_ne_zero ( by aesop_cat ), h_cycle_rep ⟩;
        grind;
      omega;
    -- Thus `f^[k] rep = f^[j] rep` iff `k ≡ j [MOD p]`.
    apply Iff.intro;
    · intro h_eq
      have h_mod : f^[k % cycle_period f rep] rep = f^[j % cycle_period f rep] rep := by
        rw [ ← Nat.mod_add_div k ( cycle_period f rep ), ← Nat.mod_add_div j ( cycle_period f rep ) ] at *; simp_all +decide [ Function.iterate_add_apply, Function.iterate_mul, Function.iterate_fixed ] ;
      by_contra h_contra;
      exact h_distinct ( Min.min ( k % cycle_period f rep ) ( j % cycle_period f rep ) ) ( Max.max ( k % cycle_period f rep ) ( j % cycle_period f rep ) ) ( lt_of_le_of_ne ( min_le_max ) ( by contrapose! h_contra; simp_all +decide [ Nat.ModEq ] ) ) ( by cases max_cases ( k % cycle_period f rep ) ( j % cycle_period f rep ) <;> linarith [ Nat.mod_lt k ( show cycle_period f rep > 0 from h_cycle_period_rep ▸ cycle_period_pos f x ), Nat.mod_lt j ( show cycle_period f rep > 0 from h_cycle_period_rep ▸ cycle_period_pos f x ) ] ) ( by cases max_cases ( k % cycle_period f rep ) ( j % cycle_period f rep ) <;> cases min_cases ( k % cycle_period f rep ) ( j % cycle_period f rep ) <;> aesop );
    · intro h; rw [ ← Nat.mod_add_div k ( cycle_period f rep ), ← Nat.mod_add_div j ( cycle_period f rep ), h_cycle_period_rep ] ; simp_all +decide [ Function.iterate_add_apply, Function.iterate_mul, Function.iterate_fixed ] ;
      rw [ h ]

/-
If x is in a cycle, its cycle position advances by 1 (modulo the period) when applying f.
-/
lemma cycle_position_f_of_in_cycle {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) (h : in_cycle f x) :
  cycle_position f (f x) ≡ cycle_position f x + 1 [MOD cycle_period f x] := by
    -- By definition of cycle_position, we know that $f^{cycle_position f x} (component_rep f x) = root f x$ and $f^{cycle_position f (f x)} (component_rep f x) = root f (f x)$.
    have h_cycle_pos : f^[cycle_position f x] (component_rep f x) = root f x ∧ f^[cycle_position f (f x)] (component_rep f x) = root f (f x) := by
      apply And.intro;
      · exact Nat.find_spec ( _ : ∃ k, f^[k] ( component_rep f x ) = root f x );
      · have h_cycle_pos_f : component_rep f (f x) = component_rep f x := by
          ...
          have h_comp_rep_fx : cycle_elements f (f x) = cycle_elements f x := by
            ext y
            simp [cycle_elements, h_root_fx];
            rw [ h_root_x, cycle_period_apply_of_in_cycle f x h ];
            constructor <;> rintro ⟨ a, ha, rfl ⟩;
            · use if a = cycle_period f x - 1 then 0 else a + 1;
              split_ifs <;> simp_all +decide [ Function.iterate_succ_apply' ];
              · have := iterate_cycle_period_root f x; cases k : cycle_period f x <;> simp_all +decide [ Function.iterate_succ_apply' ] ;
            ...
            · rcases a with ( _ | a ) <;> simp_all +decide [ Function.iterate_succ_apply' ];
              · use cycle_period f x - 1;
                have := iterate_cycle_period_root f x; rcases k : cycle_period f x with ( _ | _ | k ) <;> simp_all +decide [ Function.iterate_succ_apply' ] ;
                simpa only [ ← Function.iterate_succ_apply' f ] using this;
              · exact ⟨ a, by linarith, by erw [ Function.iterate_succ_apply' ] ⟩;
          unfold component_rep; aesop;
        rw [ ← h_cycle_pos_f, cycle_position ];
        grind;
    ...
    have h_root_fx_eq : f^[cycle_position f (f x)] (component_rep f x) = f^[cycle_position f x + 1] (component_rep f x) := by
      simp_all +decide [ Function.iterate_succ_apply' ];
      rw [ root_eq_self_of_in_cycle f x h ];
    exact
      (iterate_eq_iff_modEq_of_component_rep f x (cycle_position f (f x))
            (cycle_position f x + 1)).mp
        h_root_fx_eq

/-
The set of cycle elements is invariant under f for elements in a cycle.
-/
lemma cycle_elements_eq_of_in_cycle {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) (h : in_cycle f x) :
  cycle_elements f (f x) = cycle_elements f x := by
    -- Since x is in a cycle, the root of x is x itself.
    have h_root_eq : root f x = x := by
      exact root_eq_self_of_in_cycle f x h;
    rw [ show cycle_elements f ( f x ) = ( Finset.range ( cycle_period f x ) ).image ( fun k => f^[k] ( root f ( f x ) ) ) from ?_, show cycle_elements f x = ( Finset.range ( cycle_period f x ) ).image ( fun k => f^[k] ( root f x ) ) from ?_ ];
    · rw [ show root f ( f x ) = f x from ?_, show root f x = x from h_root_eq ];
      · ext; simp +decide ;
        constructor <;> rintro ⟨ a, ha, rfl ⟩;
        · by_cases ha' : a + 1 < cycle_period f x;
          · exact ⟨ a + 1, ha', by simp +decide ⟩;
          · use 0;
            cases lt_or_eq_of_le ( Nat.le_of_not_lt ha' ) <;> simp_all +decide;
            · linarith;
            · have := iterate_cycle_period_root f x; aesop;
        · use if a = 0 then cycle_period f x - 1 else a - 1;
...
-/
lemma jp_linear {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (p : ℕ) (zeta : ZMod p) (x : ZMod n) (h_zeta : zeta ^ (cycle_period f x) = 1) :
  jp f p zeta (f x) = zeta * jp f p zeta x := by
    by_cases hx : in_cycle f x <;> simp_all +decide [ jp ];
    · -- Since $x$ is in a cycle, we have $root f x = x$ and $depth f x = 0$.
      have h_root_depth : root f x = x ∧ depth f x = 0 := by
        ...
        unfold depth; aesop;
      have h_cycle_pos : cycle_position f (f x) ≡ cycle_position f x + 1 [MOD cycle_period f x] := by
        exact cycle_position_f_of_in_cycle f x hx;
      have h_cycle_pos_eq : zeta ^ cycle_position f (f x) = zeta ^ (cycle_position f x + 1) := by
        rw [ ← Nat.mod_add_div ( cycle_position f ( f x ) ) ( cycle_period f x ), ← Nat.mod_add_div ( cycle_position f x + 1 ) ( cycle_period f x ), h_cycle_pos ] ; simp_all +decide [ pow_add, pow_mul ];
      have h_depth_f : depth f (f x) = 0 := by
        have h_depth_f : in_cycle f (f x) := by
          obtain ⟨ k, hk ⟩ := hx;
          exact ⟨ k, hk.1, by erw [ Function.iterate_succ_apply', hk.2 ] ⟩;
...
def jq {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (q N : ℕ) (x : ZMod n) : ℕ :=
  (Finset.range (depth f x)).sum (fun k => (signature_map f (f^[k] x) + 1) * q ^ (N - 1 - k))
...
lemma jq_mod_linear {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (q N : ℕ) (x : ZMod n) (hN : N ≥ depth f x) :
  (jq f q N (f x) : ZMod (q ^ N)) = (q : ZMod (q ^ N)) * jq f q N x := by
    unfold jq;
    by_cases h : in_cycle f x <;> simp_all +decide;
    · -- Since x is in a cycle, depth f x = 0.
      have h_depth_zero : depth f x = 0 := by
        rw [ depth ];
        aesop;
...
-/
lemma jq_step {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (q N : ℕ) (x : ZMod n) (h_not_cycle : ¬ in_cycle f x) (hN : N ≥ depth f x) :
  q * jq f q N x = (signature_map f x + 1) * q ^ N + jq f q N (f x) := by
...
lemma jq_inj {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (q N : ℕ) (x y : ZMod n)
  ...
  x = y := by
    ...
    by_cases hx : in_cycle f x <;> by_cases hy : in_cycle f y;
    ...
    · have h_depth_y : depth f y = 0 := by
      ...
      grind;
    · -- Since $x$ and $y$ are not in cycles, we have $jq f q N x = (signature_map f x + 1) * q ^ N + jq f q N (f x)$ and $jq f q N y = (signature_map f y + 1) * q ^ N + jq f q N (f y)$.
      have h_jq_step : q * jq f q N x = (signature_map f x + 1) * q ^ N + jq f q N (f x) ∧ q * jq f q N y = (signature_map f y + 1) * q ^ N + jq f q N (f y) := by
        apply And.intro;
        · apply jq_step;
          · assumption;
          · -- Since `depth f x` is the smallest `k` such that `f^[k] x` is in a cycle, and `x` is not in a cycle, `depth f x` must be less than or equal to `n`.
            have h_depth_le_n : depth f x ≤ n := by
              have h_depth_le_n : ∃ k ≤ n, in_cycle f (f^[k] x) := by
        ...
        · convert jq_step f q N y hy _ using 1;
          have h_depth_le_n : ∀ x : ZMod n, depth f x ≤ n := by
            intro x;
            have h_depth_le_n : ∀ x : ZMod n, ∃ k ≤ n, in_cycle f (f^[k] x) := by
              intro x
      ...
      have h_jq_lt_qN : jq f q N (f x) < q ^ N ∧ jq f q N (f y) < q ^ N := by
        ...
        · apply jq_bound;
          ...
          · rw [ depth_f_of_not_in_cycle f y hy ];
            ...
            omega;
      -- Since $q > n$, we have $signature_map f x = signature_map f y$.
      have h_signature_eq : signature_map f x = signature_map f y := by
        nlinarith [ pow_pos ( by linarith : 0 < q ) N ];
      -- Since $signature_map f x = signature_map f y$, we have $f x = f y$.
...
lemma jq_valuation {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (q N : ℕ) (x : ZMod n)
  ...
  (q ^ (N - depth f x)) ∣ jq f q N x ∧ ¬ (q ^ (N - depth f x + 1)) ∣ jq f q N x := by
    ...
    have h_jq_sum :jq f q N x = (signature_map f (f^[depth f x - 1] x) + 1) * q^(N - depth f x) + ∑ k ∈ Finset.range (depth f x - 1), (signature_map f (f^[k] x) + 1) * q^(N - 1 - k) := by
    ...
    have h_sum_div : q ^ (N - depth f x + 1) ∣ ∑ k ∈ Finset.range (depth f x - 1), (signature_map f (f^[k] x) + 1) * q^(N - 1 - k) := by
...
lemma j_combined_linear {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (p q N : ℕ) (zeta : ZMod p) (x : ZMod n)
  (hpq : Nat.Coprime p (q ^ N)) (h_zeta : zeta ^ (cycle_period f x) = 1)
...
noncomputable def j_map {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (p q N : ℕ) (zeta : ZMod p) (hpq : Nat.Coprime p (q ^ N)) (x : ZMod n) : ZMod (p * q ^ N) :=
...
lemma j_map_linear {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (p q N : ℕ) (zeta : ZMod p) (x : ZMod n)
  ...
  j_map f p q N zeta hpq (f x) = multiplier p q N zeta hpq * j_map f p q N zeta hpq x := by
...
lemma exists_valid_primes {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) :
  ∃ P Q : {r // r ∈ components f} → ℕ,
    ...
    (∀ r, P r > n) ∧ (∀ r, Q r > n) ∧
    (∀ r, P r ≡ 1 [MOD cycle_period f r.1]) ∧
    Function.Injective P ∧ Function.Injective Q ∧
    (∀ r1 r2, P r1 ≠ Q r2) := by
      simp +zetaDelta at *;
      have h_inf_primes : Set.Infinite {p : ℕ | Nat.Prime p ∧ p > n ∧ ∀ r ∈ components f, p ≡ 1 [MOD cycle_period f r]} := by
        have h_inf_primes : Set.Infinite {p : ℕ | Nat.Prime p ∧ p ≡ 1 [MOD (∏ r ∈ components f, cycle_period f r)]} := by
          apply_rules [ Nat.infinite_setOf_prime_modEq_one ];
          exact Finset.prod_ne_zero_iff.mpr fun x hx => Nat.ne_of_gt <| Nat.pos_of_ne_zero <| by unfold cycle_period; aesop;
      ...
      · exact fun a b => ( ‹∀ x ∈ t, Nat.Prime x ∧ n < x ∧ ∀ r ∈ components f, x ≡ 1 [MOD cycle_period f r]› _ ( ht₁ _ |>.1 ( Or.inl ( hP.2 a b ) ) ) ) |>.1;
      · exact fun a ha => by have := hQ.2 a ha; specialize ht₁ ( Q ⟨ a, ha ⟩ ) ; aesop;
      · exact fun a ha => ( ‹∀ x ∈ t, Nat.Prime x ∧ n < x ∧ ∀ r ∈ components f, x ≡ 1 [MOD cycle_period f r]› _ ( ht₁ _ |>.1 ( Or.inl ( hP.2 _ ha ) ) ) ) |>.2.1;
      · exact fun a ha => ( ‹∀ x ∈ t, Nat.Prime x ∧ n < x ∧ ∀ r ∈ components f, x ≡ 1 [MOD cycle_period f r]› _ ( ht₁ _ |>.1 ( Or.inr ( hQ.2 _ ha ) ) ) ) |>.2.1;
      · exact ⟨ fun a ha => by have := ‹∀ x ∈ t, Nat.Prime x ∧ n < x ∧ ∀ r ∈ components f, x ≡ 1 [MOD cycle_period f r]› ( P ⟨ a, ha ⟩ ) ( ht₁ _ |>.1 ( Or.inl ( hP.2 a ha ) ) ) ; aesop, fun a ha b hb => fun h => ht₁₂ _ ( hP.2 a ha ) ( h.symm ▸ hQ.2 b hb ) ⟩

/-
Equality of j_map implies equality of its components jp and jq.
-/
lemma j_map_eq_implies_parts {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (p q N : ℕ) (zeta : ZMod p)
  ...
  (h_eq : j_map f p q N zeta hpq x = j_map f p q N zeta hpq y) :
  jp f p zeta x = jp f p zeta y ∧ jq f q N x = jq f q N y := by
    -- Since the Chinese Remainder Theorem isomorphism is injective, if $j_map x = j_map y$, then their components must be equal.
...
lemma cycle_position_lt_cycle_period {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) :
  cycle_position f x < cycle_period f x := by
    have h_exists_k : ∃ k, f^[k] (component_rep f x) = root f x ∧ k < cycle_period f x := by
      -- By definition of `Nat.find`, we know that `root f x` is in the cycle elements and thus there exists some `k` such that `f^[k] (component_rep f x) = root f x`.
      obtain ⟨k, hk⟩ : ∃ k, f^[k] (component_rep f x) = root f x := by
        exact ⟨ cycle_position f x, by exact Nat.find_spec ( _ : ∃ k, f^[k] ( component_rep f x ) = root f x ) ⟩;
      refine' ⟨ k % cycle_period f x, _, _ ⟩;
      · rw [ ← hk, ← Nat.mod_add_div k ( cycle_period f x ), Function.iterate_add_apply ];
        induction k / cycle_period f x <;> simp_all +decide [ Nat.mul_succ, ← Function.iterate_add_apply ];
        simp_all +decide [ ← add_assoc, Nat.add_mod ];
        -- By definition of cycle_period, we know that f^[cycle_period f x] (component_rep f x) = component_rep f x.
        have h_cycle : f^[cycle_period f x] (component_rep f x) = component_rep f x := by
          have h_root : f^[cycle_period f x] (root f x) = root f x := by
            exact iterate_cycle_period_root f x
          have h_root : component_rep f x ∈ cycle_elements f x := by
            exact component_rep_mem_cycle f x;
...
lemma root_eq_of_cycle_position_eq {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x y : ZMod n)
  (h_comp : component_rep f x = component_rep f y)
  (h_pos : cycle_position f x = cycle_position f y) :
  root f x = root f y := by
    ...
    have h_cycle_pos_eq : f^[cycle_position f x] (component_rep f x) = root f x ∧ f^[cycle_position f y] (component_rep f y) = root f y := by
      exact ⟨ Nat.find_spec ( _ : ∃ k, f^[k] ( component_rep f x ) = root f x ), Nat.find_spec ( _ : ∃ k, f^[k] ( component_rep f y ) = root f y ) ⟩;
    aesop
...
lemma cycle_position_eq_of_jp_eq {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (p : ℕ) (zeta : ZMod p) (x y : ZMod n)
  (h_comp : component_rep f x = component_rep f y)
  (h_zeta_order : orderOf zeta = cycle_period f x)
  (h_depth : depth f x = depth f y)
  (h_jp : jp f p zeta x = jp f p zeta y) :
  cycle_position f (root f x) = cycle_position f (root f y) := by
    have h_jp_exp : zeta ^ (cycle_position f (root f x)) * (zeta⁻¹) ^ (depth f x) = zeta ^ (cycle_position f (root f y)) * (zeta⁻¹) ^ (depth f y) := by
      exact h_jp;
    by_cases h : IsUnit zeta <;> simp_all +decide;
    · have h_cycle_pos_eq : zeta ^ (cycle_position f (root f x)) = zeta ^ (cycle_position f (root f y)) := by
        have h_inv : IsUnit (zeta⁻¹ ^ depth f y) := by
          exact IsUnit.pow _ ( by cases h ; aesop );
        exact h_inv.mul_left_inj.mp h_jp_exp;
      have h_cycle_pos_eq : cycle_position f (root f x) ≡ cycle_position f (root f y) [MOD orderOf zeta] := by
        rw [ Nat.modEq_iff_dvd ];
        rw [ dvd_sub_comm ];
        have h_cycle_pos_eq : zeta ^ (cycle_position f (root f x) - cycle_position f (root f y)) = 1 := by
          cases le_total ( cycle_position f ( root f x ) ) ( cycle_position f ( root f y ) ) <;> simp_all +decide;
          cases le_iff_exists_add'.mp ‹_› ; simp_all +decide [ pow_add ];
          have h_cycle_pos_eq : IsUnit (zeta ^ cycle_position f (root f y)) := by
            exact h.pow _;
          exact h_cycle_pos_eq.mul_left_inj.mp ( by linear_combination' ‹zeta ^ _ * zeta ^ cycle_position f ( root f y ) = zeta ^ cycle_position f ( root f y ) › );
        cases le_total ( cycle_position f ( root f x ) ) ( cycle_position f ( root f y ) ) <;> simp_all +decide;
        · have h_cycle_pos_eq : zeta ^ (cycle_position f (root f y) - cycle_position f (root f x)) = 1 := by
            have h_cycle_pos_eq : zeta ^ (cycle_position f (root f y) - cycle_position f (root f x)) * zeta ^ (cycle_position f (root f x)) = zeta ^ (cycle_position f (root f x)) := by
              rw [ ← pow_add, Nat.sub_add_cancel ‹_›, h_cycle_pos_eq ];
            have h_cycle_pos_eq : IsUnit (zeta ^ cycle_position f (root f x)) := by
              exact h.pow _;
            exact h_cycle_pos_eq.mul_left_cancel ( by linear_combination' ‹zeta ^ ( cycle_position f ( root f y ) - cycle_position f ( root f x ) ) * zeta ^ cycle_position f ( root f x ) = zeta ^ cycle_position f ( root f x ) › );
      ...
      have h_cycle_pos_eq : cycle_position f (root f x) < cycle_period f x ∧ cycle_position f (root f y) < cycle_period f x := by
        have h_cycle_pos_eq : ∀ x, cycle_position f (root f x) < cycle_period f x := by
          intros x; exact (by
        ...
        have h_cycle_pos_eq : cycle_period f x = cycle_period f y := by
...
/-
j_map is injective for elements in the same component.
-/
lemma j_map_injective {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (p q N : ℕ) (zeta : ZMod p)
  ...
  have h_pos : cycle_position f (root f x) = cycle_position f (root f y) := by
  ...
  have h_root : root f x = root f y := by
    ...
    have h_root_eq : ∀ {x y : ZMod n}, in_cycle f x → in_cycle f y → component_rep f x = component_rep f y → cycle_position f x = cycle_position f y → x = y := by
    ...
    · have h_root_comp : component_rep f x = component_rep f (root f x) ∧ component_rep f y = component_rep f (root f y) := by
        have h_root_comp : ∀ x, component_rep f x = component_rep f (root f x) := by
          intros x
          apply Eq.symm;
          have h_root_cycle : cycle_elements f (root f x) = cycle_elements f x := by
            unfold cycle_elements;
...
lemma jp_is_unit {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (p : ℕ) (zeta : ZMod p) (x : ZMod n)
  ...
  IsUnit (jp f p zeta x) := by
    ...
    have h_unit : IsUnit zeta := by
      have h_unit : zeta ^ cycle_period f x = 1 := by
        rw [ ← h_zeta_order, pow_orderOf_eq_one ];
      exact isUnit_iff_dvd_one.mpr ( h_unit ▸ dvd_pow_self _ ( by linarith [ cycle_period_pos f x ] ) );
    obtain ⟨ k, hk ⟩ := h_unit;
    exact h_not_unit <| by rw [ show jp f p zeta x = zeta ^ ( cycle_position f ( root f x ) ) * zeta⁻¹ ^ ( depth f x ) by rfl ] ; exact IsUnit.mul ( IsUnit.pow _ <| by aesop ) ( IsUnit.pow _ <| by aesop ) ;
...
lemma component_rep_apply {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) :
  component_rep f (f x) = component_rep f x := by
    -- Consider two cases: when x is in a cycle and when it is not.
    by_cases hx_cycle : in_cycle f x;
    · unfold component_rep;
      -- Since $x$ is in the cycle, we have $cycle_elements f (f x) = cycle_elements f x$.
      have h_cycle_elements_eq : cycle_elements f (f x) = cycle_elements f x := by
    ...
    · -- Since x is not in a cycle, its root is the same as the root of f(x).
      ...
      have h_cycle_elements_eq : cycle_elements f x = cycle_elements f (f x) := by
        unfold cycle_elements;
        unfold cycle_period at * ; aesop;
      unfold component_rep; aesop;
...
-/
def global_j {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (P Q : {r // r ∈ components f} → ℕ)
  (zeta : Π r, ZMod (P r))
  ...
  (ZMod.prodEquivPi (fun r => P r * (Q r) ^ n)
    ...
    if _h : r.val = component_rep f x then
      j_map f (P r) (Q r) n (zeta r) (hpq r) x
    else
      0)

def global_multiplier {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (P Q : {r // r ∈ components f} → ℕ)
  (zeta : Π r, ZMod (P r))
  (hpq : ∀ r, Nat.Coprime (P r) ((Q r) ^ n))
  (hP : ∀ r, (P r).Prime) (hQ : ∀ r, (Q r).Prime)
  (h_disjoint : ∀ r1 r2, P r1 ≠ Q r2)
...
lemma depth_le_n {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (x : ZMod n) : depth f x ≤ n := by
  ...
  obtain ⟨k, hk⟩ : ∃ k, in_cycle f (f^[k] x) ∧ k ≤ n := by
...
def global_iso {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (P Q : {r // r ∈ components f} → ℕ)
  (hP : ∀ r, (P r).Prime) (hQ : ∀ r, (Q r).Prime)
  (h_disjoint : ∀ r1 r2, P r1 ≠ Q r2)
  (hP_inj : Function.Injective P) (hQ_inj : Function.Injective Q) :
...
-/
def global_m {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (P Q : {r // r ∈ components f} → ℕ) : ℕ :=
  ∏ r, P r * (Q r) ^ n
...
lemma global_j_injective {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (P Q : {r // r ∈ components f} → ℕ)
  ...
  (h_zeta : ∀ r, orderOf (zeta r) = cycle_period f r.1) :
  Function.Injective (global_j f P Q zeta hpq hP hQ h_disjoint hP_inj hQ_inj) := by
    -- By definition of `global_j`, if `global_j x = global_j y`, then for every component `r`, the `j_map` values for `x` and `y` must be equal.
    intro x y h_eq
    have h_comp : component_rep f x = component_rep f y := by
      contrapose! h_eq;
      unfold global_j;
      -- Since component_rep f x ≠ component_rep f y, there exists some component r where the j_map values are different.
      obtain ⟨r, hr⟩ : ∃ r : { r // r ∈ components f }, (r.val = component_rep f x) ∧ (r.val ≠ component_rep f y) := by
        exact ⟨ ⟨ component_rep f x, Finset.mem_image.mpr ⟨ x, Finset.mem_univ _, rfl ⟩ ⟩, rfl, h_eq ⟩;
    ...
    have h_jp : ∀ r : { r // r ∈ components f }, r.val = component_rep f x → j_map f (P r) (Q r) n (zeta r) (hpq r) x = j_map f (P r) (Q r) n (zeta r) (hpq r) y := by
      intro r hr
      have h_eq_j_map : (global_iso f P Q hP hQ h_disjoint hP_inj hQ_inj).symm (fun r => if h : r.val = component_rep f x then j_map f (P r) (Q r) n (zeta r) (hpq r) x else 0) = (global_iso f P Q hP hQ h_disjoint hP_inj hQ_inj).symm (fun r => if h : r.val = component_rep f y then j_map f (P r) (Q r) n (zeta r) (hpq r) y else 0) := by
        exact h_eq;
      replace h_eq_j_map := congr_arg ( fun z => ( global_iso f P Q hP hQ h_disjoint hP_inj hQ_inj ) z r ) h_eq_j_map
      simp only [RingEquiv.apply_symm_apply, dif_pos hr, dif_pos (hr.trans h_comp)] at h_eq_j_map
      exact h_eq_j_map
    apply j_map_injective f (P ⟨component_rep f x, by
      exact Finset.mem_image_of_mem _ ( Finset.mem_univ _ )⟩) (Q ⟨component_rep f x, by
      exact Finset.mem_image_of_mem _ ( Finset.mem_univ _ )⟩) n (zeta ⟨component_rep f x, by
      exact Finset.mem_image_of_mem _ ( Finset.mem_univ _ )⟩) (hpq ⟨component_rep f x, by
      exact Finset.mem_image_of_mem _ ( Finset.mem_univ _ )⟩) (by
    exact hQ_gt _) (by
    norm_num) (by
    exact fun z => depth_le_n f z) x y h_comp (by
    all_goals generalize_proofs at *;
    convert h_zeta ⟨ component_rep f x, by assumption ⟩ using 1
    generalize_proofs at *;
    -- Since component_rep f x is in the cycle elements of x, and component_rep f x is in the cycle elements of component_rep f x, the cycle periods must be equal.
    have h_cycle_period_eq : cycle_period f x = cycle_period f (component_rep f x) := by
      have h_cycle_elements : component_rep f x ∈ cycle_elements f x := by
        exact component_rep_mem_cycle f x
      exact cycle_period_eq_of_mem_cycle_elements f x (component_rep f x) h_cycle_elements;
    exact h_cycle_period_eq) (h_jp ⟨component_rep f x, by
      exact Finset.mem_image_of_mem _ ( Finset.mem_univ _ )⟩ rfl)

/-
Linearity of global_j.
-/
lemma global_j_linear {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) (P Q : {r // r ∈ components f} → ℕ)
  ...
  (_hP_gt : ∀ r, P r > n) (hQ_gt : ∀ r, Q r > n)
  (h_zeta : ∀ r, orderOf (zeta r) = cycle_period f r.1) :
  ∀ x, global_j f P Q zeta hpq hP hQ h_disjoint hP_inj hQ_inj (f x) =
       global_multiplier f P Q zeta hpq hP hQ h_disjoint hP_inj hQ_inj *
       global_j f P Q zeta hpq hP hQ h_disjoint hP_inj hQ_inj x := by
         intro x;
         apply (global_iso f P Q hP hQ h_disjoint hP_inj hQ_inj).injective;
         ext r;
         by_cases hr : r.val = component_rep f x
         ...
         · simp only [global_j, global_multiplier, global_iso, map_mul, Pi.mul_apply,
             RingEquiv.apply_symm_apply, component_rep_apply, dif_pos hr];
           apply j_map_linear;
           · have := h_zeta r;
             -- Since $r.val = component_rep f x$, we have $cycle_period f r.val = cycle_period f (component_rep f x)$.
             have h_cycle_period_eq : cycle_period f r.val = cycle_period f (component_rep f x) := by
               rw [ hr ];
             rw [ ← orderOf_dvd_iff_pow_eq_one ];
             rw [ this, h_cycle_period_eq ];
             have h_cycle_period_eq : component_rep f x ∈ cycle_elements f x := by
               exact component_rep_mem_cycle f x;
             rw [ cycle_period_eq_of_mem_cycle_elements f x ( component_rep f x ) h_cycle_period_eq ];
           · exact hQ_gt r;
           · norm_num;
...
-/
theorem linear_representation {n : ℕ} [NeZero n] (f : ZMod n → ZMod n) : has_linear_representation f := by
  ...
  obtain ⟨P, Q, hP, hQ, h_disjoint, hP_inj, hQ_inj⟩ : ∃ P Q : {r // r ∈ R} → ℕ, (∀ r, (P r).Prime) ∧ (∀ r, (Q r).Prime) ∧ (∀ r, P r > n) ∧ (∀ r, Q r > n) ∧ (∀ r, P r ≡ 1 [MOD cycle_period f r.1]) ∧ Function.Injective P ∧ Function.Injective Q ∧ (∀ r1 r2, P r1 ≠ Q r2) := by
    exact exists_valid_primes f;
  -- Let's choose a primitive root of unity `zeta` for each component.
  obtain ⟨zeta, hzeta⟩ : ∃ zeta : Π r : {r // r ∈ R}, ZMod (P r), ∀ r, orderOf (zeta r) = cycle_period f r.1 := by
    have h_primitive_root : ∀ r : {r // r ∈ R}, ∃ zeta : ZMod (P r), orderOf zeta = cycle_period f r.1 := by
      intro r;
      have := hQ_inj.1 r;
      have := exists_primitive_root_of_unity ( hP r ) ( show 0 < cycle_period f r from ?_ ) this;
      · exact this;
  ...
  use global_m f P Q, global_multiplier f P Q zeta (fun r => Nat.coprime_iff_gcd_eq_one.mpr (by
  exact Nat.Coprime.pow_right _ ( Nat.coprime_iff_gcd_eq_one.mpr <| by have := Nat.coprime_primes ( hP r ) ( hQ r ) ; aesop ))) hP hQ hQ_inj.right.right.right (hQ_inj.right.left) (hQ_inj.right.right.left), global_j f P Q zeta (fun r => Nat.coprime_iff_gcd_eq_one.mpr (by

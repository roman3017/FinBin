...
def represented_by_polynomial_of_degree_in (n : ℕ) (f : α × α → α) (d : ℕ) : Prop :=
  ∃ g : MvPolynomial (Fin 2) (ZMod n),
    represents f (fun v => g.eval ![v.1, v.2]) ∧
    g.totalDegree ≤ d

def represented_by_polynomial_of_degree (f : α × α → α) (d : ℕ) : Prop :=
  ∃ n : ℕ, represented_by_polynomial_of_degree_in n f d

def kronecker_delta (n : ℕ) [NeZero n] (a b : ZMod n) : ZMod n × ZMod n → ZMod n :=
  fun p => if p.1 = a ∧ p.2 = b then 1 else 0

theorem quadratic_d2 (a b : ZMod 2) :
  represented_by_polynomial_of_degree (kronecker_delta 2 a b) 2 := by
    ...
    · use ⟨ fun x => x, by
      ...
      intro x y
      fin_cases a <;> fin_cases b <;> fin_cases x <;> fin_cases y <;> simp +decide [ kronecker_delta ] ;
...
theorem cubic_d3_02 : represented_by_polynomial_of_degree (kronecker_delta 3 0 2) 3 := by
  use 4;
  ...
  · use ⟨ fun x => if x = 0 then 0 else if x = 1 then 2 else 1, by decide ⟩
    ...
    simp only [kronecker_delta, Function.Embedding.coeFn_mk]
...
theorem quadratic_d_00 (n : ℕ) [NeZero n] :
    represented_by_polynomial_of_degree (kronecker_delta n 0 0) 2 := by
  by_contra h;
...
theorem quadratic_d_11 (n : ℕ) [NeZero n] :
    represented_by_polynomial_of_degree (kronecker_delta n 1 1) 2 := by
  ...
  have hG_rep : ∀ x y, j' (kronecker_delta n 1 1 (x, y)) = G.eval ![j' x, j' y] := by
    intro x y
    simp [G, j'];
    convert congr_arg ( fun z => -z + ( j 0 + j 1 ) ) ( hj ( if x = 0 then 1 else if x = 1 then 0 else x ) ( if y = 0 then 1 else if y = 1 then 0 else y ) ) using 1 ; simp +decide [ kronecker_delta ] ; ring_nf!; (
    grind)
...
theorem quadratic_d_22 (n : ℕ) [NeZero n] :
    represented_by_polynomial_of_degree (kronecker_delta n 2 2) 2 := by
  by_cases hn : n < 3;
  ...
  · have hn_ge3 : n ≥ 3 := by linarith
    ...
    have h_ge3_case : represented_by_polynomial_of_degree (kronecker_delta n 2 2) 2 := by
      use 4 * n, MvPolynomial.C (2 * n : ZMod (4 * n)) * MvPolynomial.X 0 * MvPolynomial.X 1, by
        ...
        use ⟨ j_map_22 n, ?_ ⟩;
        intro x y; specialize h_rep x y; simp_all +decide [ kronecker_delta ] ;
...
def m_val_12 (n : ℕ) : ℕ := if n % 2 = 1 then n else n + 1

def N_val_12 (n : ℕ) : ℕ := if n = 1 then 1 else 4 * m_val_12 n

theorem cubic_d_12 (n : ℕ) [NeZero n] :
    represented_by_polynomial_of_degree (kronecker_delta n 1 2) 3 := by
  let N_val_final : ℕ := if n = 2 then 3 else N_val_12 n
  let j_map_fun_12 : ZMod n → ZMod (N_val_12 n) :=
    fun x =>
      if n = 1 then 0
      else
        let m := m_val_12 n
  ...
  let poly_G_12 : MvPolynomial (Fin 2) (ZMod (N_val_12 n)) :=
    if n = 1 then 0
    else
      let m : ZMod (N_val_12 n) := m_val_12 n
  ...
  have j_map_injective : Function.Injective j_map_fun_12 := by
    ...
    · intros a₁ a₂ h_eq
      by_cases ha₁ : a₁ = 1 ∨ a₁ = 2
      by_cases ha₂ : a₂ = 1 ∨ a₂ = 2;
      · rcases ha₁ with ( rfl | rfl ) <;> rcases ha₂ with ( rfl | rfl ) <;> simp_all +decide [ m_val_12 ];
        · split_ifs at h_eq <;> norm_cast at h_eq;
          · erw [ ZMod.natCast_eq_natCast_iff ] at h_eq ; simp_all +decide [ Nat.ModEq ];
            rcases n with ( _ | _ | _ | n ) <;> simp_all +arith +decide [ N_val_12 ];
            have := h_eq ( by rintro ⟨ ⟩ ) ; rw [ Nat.mod_eq_of_lt, Nat.mod_eq_of_lt ] at this <;> simp_all +arith +decide [ m_val_12 ] ;
          · erw [ ZMod.natCast_eq_natCast_iff ] at h_eq ; simp_all +decide [ Nat.ModEq ];
            rcases n with ( _ | _ | _ | _ | _ | n ) <;> simp_all +arith +decide [ N_val_12 ];
            have := h_eq ( by rintro ⟨ ⟩ ) ; rw [ Nat.mod_eq_of_lt, Nat.mod_eq_of_lt ] at this <;> linarith [ show m_val_12 ( n + 5 ) > n + 5 from by { unfold m_val_12; split_ifs <;> omega } ] ;
        · rcases Nat.even_or_odd' n with ⟨ k, rfl | rfl ⟩ <;> norm_num [ ZMod, N_val_12 ] at *;
          · rcases k with ( _ | _ | k ) <;> norm_cast at * ; simp_all +arith +decide ;
            ...
            · erw [ eq_comm, ZMod.natCast_eq_natCast_iff ] at h_eq ; norm_num [ Nat.ModEq, Nat.mul_mod ] at h_eq;
              unfold N_val_12 at h_eq ; norm_num [ Nat.add_mod, Nat.mul_mod ] at h_eq;
              unfold m_val_12 at h_eq ; norm_num [ Nat.add_mod, Nat.mul_mod ] at h_eq;
              simp +arith +decide [ Nat.mod_eq_of_lt ] at h_eq;
              cases h_eq;
          · rcases k with ( _ | _ | k ) <;> norm_cast at *;
            erw [ eq_comm, ZMod.natCast_eq_natCast_iff ] at h_eq ; norm_num [ Nat.ModEq, Nat.mul_mod ] at h_eq;
            unfold N_val_12 at h_eq ; norm_num [ Nat.mod_eq_of_lt ] at h_eq;
            unfold m_val_12 at h_eq ; norm_num [ Nat.mod_eq_of_lt ] at h_eq;
            exact absurd ( h_eq ( by rintro ⟨ ⟩ ) ) ( by rw [ Nat.mod_eq_of_lt ] <;> linarith );
      · split_ifs at h_eq <;> simp_all +decide ;
        · have h_mod : 2 * (m_val_12 n : ℕ) ≡ 4 * a₂.val [MOD 4] := by
            have h_mod : 2 * (m_val_12 n : ℕ) ≡ 4 * a₂.val [MOD N_val_12 n] := by
              simp_all +decide [ ← ZMod.natCast_eq_natCast_iff ];
            exact h_mod.of_dvd <| by unfold N_val_12; aesop;
          rcases Nat.even_or_odd' ( m_val_12 n ) with ⟨ k, hk | hk ⟩ <;> simp_all +decide [ Nat.ModEq, Nat.mul_mod ];
          · unfold m_val_12 at hk; split_ifs at hk <;> omega;
          · grind +ring;
        · have h_eq_mod : 3 ≡ 4 * a₂.val [MOD N_val_12 n] := by
            simp_all +decide [ ← ZMod.natCast_eq_natCast_iff ];
          have := congr_arg ( · % 4 ) h_eq_mod; norm_num [ Nat.ModEq, Nat.mul_mod ] at this;
          rcases n with ( _ | _ | _ | n ) <;> simp_all +arith +decide [ N_val_12 ];
      · by_cases ha₂ : a₂ = 1 ∨ a₂ = 2 <;> simp_all +decide [ not_or ];
        · have h_contra : 4 * a₁.val ≡ 2 * m_val_12 n [MOD N_val_12 n] ∨ 4 * a₁.val ≡ 3 [MOD N_val_12 n] := by
            split_ifs at h_eq <;> simp_all +decide [ ← ZMod.natCast_eq_natCast_iff ];
          rcases h_contra with h | h <;> have := congr_arg ( · % 4 ) h <;> norm_num [ Nat.ModEq, Nat.mul_mod ] at this ⊢ <;> rcases Nat.even_or_odd' ( m_val_12 n ) with ⟨ k, hk | hk ⟩ <;> simp_all +decide [ Nat.ModEq, Nat.add_mod, Nat.mul_mod ] ;
          · unfold m_val_12 at hk; split_ifs at hk <;> omega;
          · unfold N_val_12 at *; simp_all +decide [ Nat.add_mod, Nat.mul_mod ] ;
            have := Nat.mod_lt k zero_lt_four; interval_cases k % 4 <;> contradiction;
          · unfold m_val_12 at hk; split_ifs at hk <;> omega;
          · unfold N_val_12 at h; simp_all +decide ;
            have := congr_arg ( · % 4 ) h; norm_num [ Nat.add_mod, Nat.mul_mod ] at this;
        · have h_cast : a₁.val ≡ a₂.val [MOD m_val_12 n] := by
            have h_cast : 4 * a₁.val ≡ 4 * a₂.val [MOD N_val_12 n] := by
              simp_all +decide [ ← ZMod.natCast_eq_natCast_iff ];
            unfold N_val_12 at h_cast; simp_all +decide [ Nat.ModEq, Nat.mul_mod_mul_left ] ;
          have h_val_eq : a₁.val = a₂.val := by
            have h_val_eq : a₁.val < m_val_12 n ∧ a₂.val < m_val_12 n := by
              exact ⟨ lt_of_lt_of_le ( ZMod.val_lt a₁ ) ( by unfold m_val_12; split_ifs <;> linarith ), lt_of_lt_of_le ( ZMod.val_lt a₂ ) ( by unfold m_val_12; split_ifs <;> linarith ) ⟩;
  ...
  have poly_G_final_degree : poly_G_final.totalDegree ≤ 3 := by
    ...
    · have h_total_degree_map : poly_G_final.totalDegree ≤ poly_G_12.totalDegree := by
      ...
      have poly_G_degree : poly_G_12.totalDegree ≤ 3 := by
        ...
        split_ifs with h
        · simp
        · show (MvPolynomial.C (m_val_12 n : ZMod (N_val_12 n)) * MvPolynomial.X 0 *
                  (MvPolynomial.X 0 + 1) * MvPolynomial.X 1 :
                  MvPolynomial (Fin 2) (ZMod (N_val_12 n))).totalDegree ≤ 3
          haveI : Fact (1 < N_val_12 n) := ⟨by unfold N_val_12; simp [h]; unfold m_val_12; split_ifs <;> omega⟩
          have h1 := MvPolynomial.totalDegree_mul
            (MvPolynomial.C (m_val_12 n : ZMod (N_val_12 n)) * MvPolynomial.X 0 * (MvPolynomial.X 0 + 1) :
              MvPolynomial (Fin 2) (ZMod (N_val_12 n)))
            (MvPolynomial.X (1 : Fin 2))
          have h2 := MvPolynomial.totalDegree_mul
            (MvPolynomial.C (m_val_12 n : ZMod (N_val_12 n)) * MvPolynomial.X 0 :
              MvPolynomial (Fin 2) (ZMod (N_val_12 n)))
            (MvPolynomial.X (0 : Fin 2) + 1)
          have h3 := MvPolynomial.totalDegree_mul
            (MvPolynomial.C (m_val_12 n : ZMod (N_val_12 n)) : MvPolynomial (Fin 2) (ZMod (N_val_12 n)))
            (MvPolynomial.X (0 : Fin 2))
          have hX0 : (MvPolynomial.X (0 : Fin 2) : MvPolynomial (Fin 2) (ZMod (N_val_12 n))).totalDegree = 1 :=
            MvPolynomial.totalDegree_X _
          have hX1 : (MvPolynomial.X (1 : Fin 2) : MvPolynomial (Fin 2) (ZMod (N_val_12 n))).totalDegree = 1 :=
            MvPolynomial.totalDegree_X _
          have hXadd : (MvPolynomial.X (0 : Fin 2) + 1 : MvPolynomial (Fin 2) (ZMod (N_val_12 n))).totalDegree ≤ 1 :=
            le_trans (MvPolynomial.totalDegree_add _ _)
              (by rw [hX0, MvPolynomial.totalDegree_one]; norm_num)
          have hC : (MvPolynomial.C (m_val_12 n : ZMod (N_val_12 n)) : MvPolynomial (Fin 2) (ZMod (N_val_12 n))).totalDegree = 0 :=
  ...
  have poly_G_final_works : representation (kronecker_delta n 1 2)
      (fun v => poly_G_final.eval ![v.1, v.2]) (⟨j_map_final, j_map_final_injective⟩) := by
    have poly_G_works_n_ne_2 : ∀ (hn : n ≠ 2),
        representation (kronecker_delta n 1 2) (fun v => poly_G_12.eval ![v.1, v.2]) (⟨j_map_fun_12, j_map_injective⟩) := by
      clear_value j_map_final poly_G_final N_val_final
      have poly_G_eval_1_2 : poly_G_12.eval ![j_map_fun_12 1, j_map_fun_12 2] = j_map_fun_12 1 := by
        have h_simp : (m_val_12 n : ZMod (N_val_12 n)) * (2 * m_val_12 n) * (2 * m_val_12 n + 1) * 3 = 2 * m_val_12 n := by
          have m_val_odd : Odd (m_val_12 n) := by
            unfold m_val_12; by_cases h : n % 2 = 1 <;> simp +decide [ * ] ;
            · exact Nat.odd_iff.mpr h;
            · grind
          have h_mod : 12 * (m_val_12 n) ^ 3 + 6 * (m_val_12 n) ^ 2 ≡ 2 * (m_val_12 n) [ZMOD 4 * (m_val_12 n)] := by
            rw [ Int.modEq_iff_dvd ];
            rcases Nat.even_or_odd' ( m_val_12 n ) with ⟨ c, d | d ⟩ <;> push_cast [ * ] <;> ring_nf;
            · exact absurd m_val_odd ( by simp +decide [ d, parity_simps ] );
            · exact ⟨ -4 - c * 15 - c ^ 2 * 12, by ring ⟩;
          norm_cast at *;
          erw [ ZMod.natCast_eq_natCast_iff ] at *;
          unfold N_val_12; ring_nf at *; aesop;
      ...
      have j_map_fun_zero_of_ne_2 : ∀ (hn : n ≠ 2), j_map_fun_12 0 = 0 := by
        ...
        rcases n with ( _ | _ | n ) <;> norm_num [ m_val_12 ] at *;
        · native_decide
        · native_decide
        · have h01 : (0 : ZMod (n + 5)) ≠ 1 := by
          ...
          refine (ZMod.natCast_eq_zero_iff (↑0) (N_val_12 (n + 1 + 1 + 1 + 1 + 1))).mpr ?_
          dsimp [N_val_12, m_val_12]; norm_num;
      have poly_G_eval_y_ne_2 : ∀ (x y : ZMod n) (hy : y ≠ 2),
        ...
        · cases hy1 <;> simp_all +decide [ j_map_fun_12 ];
          ...
          · have h_mod : (m_val_12 n : ℕ) * (2 * m_val_12 n) * (2 * m_val_12 n + 1) * (2 * m_val_12 n) ≡ 0 [MOD 4 * m_val_12 n] := by
              exact Nat.modEq_zero_iff_dvd.mpr ⟨ m_val_12 n ^ 2 * ( 2 * m_val_12 n + 1 ), by ring ⟩;
            convert h_mod using 1;
            rw [ ← ZMod.natCast_eq_natCast_iff ] ; norm_num [ N_val_12 ] ; ring_nf;
            rw [ show N_val_12 n = 4 * m_val_12 n from ?_ ];
            unfold N_val_12; aesop;
          · norm_cast at *;
            rw [ ZMod.natCast_eq_zero_iff ];
            exact ⟨ 6 * m_val_12 n, by rw [ show N_val_12 n = 4 * m_val_12 n by unfold N_val_12; aesop ] ; ring_nf ⟩;
          · have h4m : (4 * m_val_12 n : ZMod (N_val_12 n)) = 0 := by
              norm_cast; rw [ ZMod.natCast_eq_zero_iff ]; unfold N_val_12; aesop;
            grind +ring;
        · unfold j_map_fun_12;
          split_ifs <;> simp_all +decide [ MvPolynomial.eval_X ];
          · have h_factor : (4 * m_val_12 n : ZMod (N_val_12 n)) = 0 := by
              norm_cast; rw [ ZMod.natCast_eq_zero_iff ] ; unfold N_val_12 ; aesop;
            grind;
          · have h_zero : (4 * m_val_12 n : ZMod (N_val_12 n)) = 0 := by
              norm_cast; rw [ ZMod.natCast_eq_zero_iff ] ; unfold N_val_12 ; aesop;
            grind;
          · have h_mod : (4 * m_val_12 n : ZMod (N_val_12 n)) = 0 := by
              norm_cast; rw [ ZMod.natCast_eq_zero_iff ] ; unfold N_val_12 ; aesop;
            grind +ring
      have poly_G_eval_y_eq_2_x_ne_1 : ∀ (x y : ZMod n) (hy : y = 2) (hx : x ≠ 1),
        ...
        grind +ring
      intro hn x y;
      by_cases hy : y = 2 <;> by_cases hx : x = 1 <;> simp_all +decide [ kronecker_delta, -j_map_fun_zero_of_ne_2, -poly_G_eval_y_eq_2_x_ne_1, -poly_G_eval_y_ne_2, -poly_G_eval_1_2 ];
    ...
    · subst hn;
      intro x y
      dsimp [j_map_final, poly_G_final, kronecker_delta];
      simp
    ...
    · convert poly_G_works_n_ne_2 hn using 1;
      ...
      · rw [ show poly_G_final = MvPolynomial.map ( ZMod.castHom ( by simp only [N_val_final, if_neg hn]; exact dvd_refl _ ) ( ZMod N_val_final ) ) poly_G_12 from ?_ ];
        · simp +decide [ MvPolynomial.eval₂_eq' ];
          ...
          have hNeq : N_val_final = N_val_12 n := if_neg hn
          rw [hNeq]
...
-/
def j_map_23_final (n : ℕ) (x : ZMod n) : ZMod (16 * n) :=
  if x = 2 then 1
  else if x = 3 then 2
  else if x = 1 then 8 * n
  else 4 * x.val

def poly_23_final (n : ℕ) : MvPolynomial (Fin 2) (ZMod (16 * n)) :=
  let X := MvPolynomial.X (0 : Fin 2)
  let Y := MvPolynomial.X (1 : Fin 2)
  (C (4 * n : ZMod (16 * n))) * X * Y * (Y - 1)

theorem cubic_d_23 (n : ℕ) [NeZero n] :
    represented_by_polynomial_of_degree (kronecker_delta n 2 3) 3 := by
  by_cases hn : n ≥ 4;
  · -- Since $n \geq 4$, we can apply the lemma `poly_23_final_represents`.
    have j_map_23_final_injective {n : ℕ} (hn : n ≥ 2) : Function.Injective (j_map_23_final n) := by
      intro x y hxy
      simp only [j_map_23_final] at hxy
      ...
      · by_cases hy : y = 2 ∨ y = 3 ∨ y = 1 <;> simp_all +decide ;
        ...
        · -- Since $4 * x.val = 4 * y.val$, we can divide both sides by 4 to get $x.val = y.val$.
          ...
          exact h_val_eq
    have j_map_23_final_injective_all : Function.Injective (j_map_23_final n) := by
      have hn2 : n ≥ 2 := by linarith
      exact j_map_23_final_injective hn2
    have poly_23_final_represents : representation (kronecker_delta n 2 3) (fun v => (poly_23_final n).eval ![v.1, v.2]) ⟨j_map_23_final n, j_map_23_final_injective_all⟩ := by
      intro x y; by_cases hx : x = 2 <;> by_cases hy : y = 3 <;> simp +decide [ hx, hy, poly_23_final ] ;
      · unfold j_map_23_final; simp +decide [ kronecker_delta ] ; ring_nf;
      ...
      · -- Since $y \neq 3$, we have $\delta_{2,3}(2, y) = 0$.
        have h_delta : kronecker_delta n 2 3 (2, y) = 0 := by
          exact if_neg ( by aesop )
        simp [h_delta] at *;
        -- Substitute the values of j_map_23_final n 0 and j_map_23_final n 2 into the equation.
        simp [j_map_23_final] at *;
      ...
      · simp +decide [ *, kronecker_delta ];
        unfold j_map_23_final; simp +decide [ hx ] ;
      ...
      · -- Since $x \neq 2$ and $y \neq 3$, the Kronecker delta $\delta_{23}(x, y)$ is 0.
        have h_delta_zero : kronecker_delta n 2 3 (x, y) = 0 := by
          exact if_neg ( by aesop );
        -- Since $x \neq 2$ and $y \neq 3$, the term $(4n) * X * Y * (Y - 1)$ will be zero because $X$ and $Y$ are not 2 or 3, respectively.
        have h_poly_zero : (4 * n : ZMod (16 * n)) * j_map_23_final n x * j_map_23_final n y * (j_map_23_final n y - 1) = 0 := by
          by_cases hy1 : y = 1 <;> simp +decide [ *, j_map_23_final ];
        ...
        rcases n with ( _ | _ | _ | _ | n ) <;> norm_cast
    use 16 * n, poly_23_final n;
    have poly_23_final_degree : (poly_23_final n).totalDegree ≤ 3 := by
      refine' le_trans _ ( _ : _ ≤ _ );
      exact ( poly_23_final n |> MvPolynomial.support |> Finset.sup <| fun x => x 0 + x 1 );
      · simp +decide [ MvPolynomial.totalDegree ];
        intro b hb; refine' le_trans _ ( Finset.le_sup <| show b ∈ MvPolynomial.support ( poly_23_final n ) from by aesop ) ; simp +decide [ Finsupp.sum_fintype ] ;
      · simp +decide [ poly_23_final ];
    ...
    exact ⟨ ⟨ ⟨ j_map_23_final n, j_map_23_final_injective_all ⟩, poly_23_final_represents ⟩, poly_23_final_degree ⟩;
  · interval_cases n <;> simp_all +decide only [represented_by_polynomial_of_degree];
    · exact False.elim <| NeZero.ne 0 rfl;
    · use 1;
      refine' ⟨ 0, _, _ ⟩ <;> norm_num [ represents ];
      exact ⟨ ⟨ fun x => x, by decide ⟩, by simp +decide [ representation, kronecker_delta ] ⟩;
    ...
    · obtain ⟨ g, hg ⟩ := cubic_d3_02;
      ...
      · obtain ⟨ j, hj ⟩ := hg.choose_spec.1;
        use j; intro x y; specialize hj y x; simp_all +decide [ kronecker_delta ] ;
...
theorem cubic_d_02 (n : ℕ) [NeZero n] :
    represented_by_polynomial_of_degree (kronecker_delta n 0 2) 3 := by
  by_cases hn : n = 1 ∨ n = 2;
  · obtain rfl | rfl := hn;
    · use 1, 0; simp +decide ;
      exists ⟨ fun _ => 0, by simp +decide ⟩;
      simp +decide [ representation, kronecker_delta ];
    · exact quadratic_d2 0 0 |> fun ⟨ n, g, h₁, h₂ ⟩ => ⟨ n, g, h₁, h₂.trans ( by norm_num ) ⟩;
  · -- Let (m, H, J) be the representation for `kronecker_delta n 1 2`.
    obtain ⟨m, H, J, hJ⟩ : ∃ m : ℕ, ∃ H : MvPolynomial (Fin 2) (ZMod m), ∃ J : ZMod n ↪ ZMod m, (∀ x y, J (kronecker_delta n 1 2 (x, y)) = H.eval ![J x, J y]) ∧ H.totalDegree ≤ 3 := by
    ...
    all_goals norm_num [ kronecker_delta ] at *;
    grind +ring;
    · intro x y; by_cases hx : x = 0 <;> by_cases hy : y = 2 <;> simp +decide [ hx, hy ] ;
      · simp +decide [ ← hJ.1, kronecker_delta ];
        simp +zetaDelta at *;
      ...
      · simp +decide [ ← hJ.1, kronecker_delta ];
        ...
        · rcases n with ( _ | _ | _ | n ) <;> cases ‹_› ; contradiction;
          tauto;
      · simp +decide [ ← hJ.1, hx, kronecker_delta ];
        grind;
      · simp +decide [ ← hJ.1, hx, hy, kronecker_delta ];
        grind;
...
def k_val_01 (n : ℕ) : ℕ := n^3 + n^2 + 1

def j_map_01 (n : ℕ) [NeZero n] (x : ZMod n) : ZMod (n * k_val_01 n) :=
  if x = 0 then 1
  else if x = 1 then 1 + n
  else (k_val_01 n : ZMod (n * k_val_01 n)) * (x.val : ZMod (n * k_val_01 n))

def poly_01 (n : ℕ) : MvPolynomial (Fin 2) (ZMod (n * k_val_01 n)) :=
  let X := MvPolynomial.X (0 : Fin 2)
  let Y := MvPolynomial.X (1 : Fin 2)
  1 + C (n : ZMod (n * k_val_01 n)) * X * Y * (X - C (1 + n : ZMod (n * k_val_01 n))) * (Y - 1)

theorem quartic_d_01 (n : ℕ) [NeZero n] :
    represented_by_polynomial_of_degree (kronecker_delta n 0 1) 4 := by
  have j_map_injective : Function.Injective (j_map_01 n) := by
    intro x y hxy;
    unfold j_map_01 at hxy;
    split_ifs at hxy <;> simp_all +decide [ ZMod.natCast_eq_zero_iff ];
    all_goals norm_cast at *;
    any_goals have := Nat.le_of_dvd ( NeZero.pos n ) hxy; rcases n with ( _ | _ | n ) <;> simp_all +decide [ k_val_01 ];
    · -- Since $k_val n$ is coprime to $n$, $k_val n * y.cast$ cannot be congruent to $1$ modulo $n * k_val n$ unless $y.cast$ is $0$.
      have h_contra : k_val_01 n * y.val ≡ 1 [MOD n * k_val_01 n] := by
        rw [ ← ZMod.natCast_eq_natCast_iff ] ; aesop;
      have := congr_arg ( · % k_val_01 n ) h_contra; norm_num [ Nat.ModEq, Nat.mul_mod, Nat.mod_eq_of_lt ] at this;
      unfold k_val_01 at this; aesop;
    · have h_contra : 1 + n ≡ k_val_01 n * y.val [MOD n * k_val_01 n] := by
        simp_all +decide [ ← ZMod.natCast_eq_natCast_iff ];
      have := congr_arg ( · % k_val_01 n ) h_contra; norm_num [ Nat.add_mod, Nat.mul_mod, Nat.mod_eq_of_lt ( show 1 + n < k_val_01 n from by { unfold k_val_01; nlinarith [ NeZero.pos n ] } ) ] at this;
      rcases n with ( _ | _ | n ) <;> simp_all +decide ;
      rw [ Nat.mod_eq_of_lt ] at this <;> norm_num [ k_val_01 ] at * ; nlinarith [ pow_succ' n 2 ];
    · replace hxy := congr_arg ( fun z => z.val ) hxy ; simp_all +decide [ ZMod.val_mul ];
      rcases n with ( _ | _ | n ) <;> simp_all +decide [ ZMod.val ];
      replace hxy := congr_arg ( fun z => z % k_val_01 ( n + 1 + 1 ) ) hxy ; simp_all +decide ;
      rw [ eq_comm, Nat.mod_eq_of_lt ] at hxy <;> norm_num [ k_val_01 ] at *;
      · cases hxy;
      · exact Nat.lt_succ_of_le ( Nat.le_add_left _ _ );
    · have h_contra : (k_val_01 n : ℕ) * x.val ≡ 1 + n [MOD n * k_val_01 n] := by
        erw [ ← ZMod.natCast_eq_natCast_iff ] ; aesop;
      have := congr_arg ( · % k_val_01 n ) h_contra; norm_num [ Nat.add_mod, Nat.mul_mod, Nat.mod_eq_of_lt ] at this;
      rcases n with ( _ | _ | n ) <;> simp +arith +decide [ Nat.mod_eq_of_lt, k_val_01 ] at *;
    ...
    · -- Since $k_val n$ is coprime to $n$, we can divide both sides of the equation by $k_val n$.
      have h_div : (x.val : ℕ) ≡ (y.val : ℕ) [MOD n] := by
        have h_div : (k_val_01 n : ℕ) * x.val ≡ (k_val_01 n : ℕ) * y.val [MOD n * k_val_01 n] := by
          simp_all +decide [ ← ZMod.natCast_eq_natCast_iff ];
        rw [ Nat.modEq_iff_dvd ] at *;
        exact h_div.imp fun a ha => by push_cast at *; nlinarith [ show 0 < k_val_01 n from Nat.pos_of_ne_zero ( by unfold k_val_01; aesop ) ] ;
      cases n <;> simp_all +decide [ ← ZMod.natCast_eq_natCast_iff ]
  have poly_01_eval (x y : ZMod n) :
      (poly_01 n).eval ![j_map_01 n x, j_map_01 n y] = j_map_01 n (kronecker_delta n 0 1 (x, y)) := by
    unfold j_map_01 poly_01 kronecker_delta;
    by_cases hx : x = 0 <;> by_cases hy : y = 0 <;> simp +decide [ hx, hy ];
    · grind;
    · split_ifs <;> simp_all +decide ;
      · rw [ neg_eq_iff_add_eq_zero ] ; ring_nf;
        norm_cast;
        rw [ ZMod.natCast_eq_zero_iff ] ; exact ⟨ 1, by unfold k_val_01; ring ⟩;
      · -- Since $n^2 * k_val n$ is a multiple of $n * k_val n$, it follows that $n^2 * k_val n \equiv 0 \pmod{n * k_val n}$.
        have h_mod : (n^2 * k_val_01 n : ZMod (n * k_val_01 n)) = 0 := by
          norm_cast;
          rw [ ZMod.natCast_eq_zero_iff ] ; exact ⟨ n, by ring ⟩;
        linear_combination' h_mod * y.cast * ( k_val_01 n * y.cast - 1 );
    · split_ifs <;> simp_all +decide [ ← mul_assoc ];
      · norm_cast ; simp +decide [ mul_comm ];
      · norm_cast ; simp +decide [ mul_comm ]
  refine' ⟨ n * ( n ^ 3 + n ^ 2 + 1 ), poly_01 n, _, _ ⟩;
  · use ⟨ j_map_01 n, j_map_injective ⟩;
    intro x y
    simpa using (poly_01_eval x y).symm
  · have poly_01_degree : (poly_01 n).totalDegree ≤ 4 := by
...
lemma kronecker_delta_perm (n : ℕ) [NeZero n] (a b : ZMod n) (σ : ZMod n ≃ ZMod n) (x y : ZMod n) :
  kronecker_delta n (σ a) (σ b) (σ x, σ y) = kronecker_delta n a b (x, y) := by
    unfold kronecker_delta; aesop;

/-
If `σ` fixes `0` and `1`, then `kronecker_delta` for permuted indices has the same polynomial representation complexity as for original indices.
-/
theorem degree_perm_fix_01 (n : ℕ) [NeZero n] (a b : ZMod n) (σ : ZMod n ≃ ZMod n) (d : ℕ)
    (h0 : σ 0 = 0) (h1 : σ 1 = 1) :
    represented_by_polynomial_of_degree (kronecker_delta n (σ a) (σ b)) d →
    represented_by_polynomial_of_degree (kronecker_delta n a b) d := by
      rintro ⟨ m, k, hk ⟩;
      refine' ⟨ m, _, _, hk.2 ⟩;
      use ⟨ fun x => hk.1.choose ( σ x ), fun x y hxy => ?_ ⟩;
      intro x y; have := hk.1.choose_spec; simp_all +decide [ representation ] ;
      · convert this ( σ x ) ( σ y ) using 1 ; simp +decide [ kronecker_delta ] ; aesop;
...
lemma represented_swap (f : α × α → α) (d : ℕ) :
  represented_by_polynomial_of_degree f d →
  represented_by_polynomial_of_degree (fun p => f (p.2, p.1)) d := by
    intro h
    obtain ⟨n, hn⟩ := h
...
lemma degree_d_eq (n : ℕ) [NeZero n] (a : ZMod n) :
  represented_by_polynomial_of_degree (kronecker_delta n a a) 2 := by
    ...
    · -- Since `n > 2` and `a \notin {0, 1, 2}`, we can use `σ = Equiv.swap a 2` to permute the indices.
      ...
      have h_perm : represented_by_polynomial_of_degree (kronecker_delta n (σ a) (σ a)) 2 := by
...
/-
If `{a, b} = {0, 1}`, then `kronecker_delta n a b` is represented by a polynomial of degree 4.
-/
lemma degree_d_01 (n : ℕ) [NeZero n] (a b : ZMod n) (h : ({a, b} : Finset (ZMod n)) = {0, 1}) :
  represented_by_polynomial_of_degree (kronecker_delta n a b) 4 := by
    ...
    · rcases h_cases with ( ⟨ rfl, rfl ⟩ | ⟨ rfl, rfl ⟩ );
      ...
      · convert represented_swap _ _ ( quartic_d_01 n ) using 1;
        exact funext fun p => by unfold kronecker_delta; aesop;
    · simp_all +decide [ Finset.ext_iff ];
      cases h a ; cases h b ; aesop

/-
For `b \notin \{0, 1\}`, `kronecker_delta n 0 b` is represented by a polynomial of degree 3.
-/
lemma degree_d_0_b (n : ℕ) [NeZero n] (b : ZMod n) (h0 : b ≠ 0) (h1 : b ≠ 1) :
  represented_by_polynomial_of_degree (kronecker_delta n 0 b) 3 := by
    by_contra h_contra;
    ...
    obtain ⟨σ, hσ⟩ : ∃ σ : ZMod n ≃ ZMod n, σ b = 2 ∧ σ 0 = 0 ∧ σ 1 = 1 := by
      ...
      · simp_all +decide [ eq_comm ];
        exact ⟨ by rintro ⟨ ⟩, by rintro ⟨ ⟩ ⟩;
    -- By Lemma `degree_perm_fix_01`, we have that `kronecker_delta n 0 b` is represented by a polynomial of degree 3.
    have h_rep : represented_by_polynomial_of_degree (kronecker_delta n (σ 0) (σ b)) 3 := by
      convert cubic_d_02 n using 1 ; aesop;
    apply h_contra;
    apply degree_perm_fix_01 n 0 b σ 3 hσ.right.left hσ.right.right h_rep

/-
For `b \notin \{0, 1\}`, `kronecker_delta n 1 b` is represented by a polynomial of degree 3.
-/
lemma degree_d_1_b (n : ℕ) [NeZero n] (b : ZMod n) (h0 : b ≠ 0) (h1 : b ≠ 1) :
  represented_by_polynomial_of_degree (kronecker_delta n 1 b) 3 := by
    by_cases hn : n ≤ 2;
    ...
    · by_contra h_contra;
      ...
      have h_perm : represented_by_polynomial_of_degree (kronecker_delta n (σ 1) (σ b)) 3 := by
        convert cubic_d_12 n using 1 ; aesop;
      exact h_contra <| degree_perm_fix_01 n 1 b σ 3 hσ.2.1 hσ.2.2 h_perm

/-
If `a, b \notin \{0, 1\}` and `a \ne b`, then `kronecker_delta n a b` is represented by a polynomial of degree 3.
-/
lemma degree_d_others (n : ℕ) [NeZero n] (a b : ZMod n)
  (ha0 : a ≠ 0) (ha1 : a ≠ 1) (hb0 : b ≠ 0) (hb1 : b ≠ 1) (hab : a ≠ b) :
  represented_by_polynomial_of_degree (kronecker_delta n a b) 3 := by
    ...
    · -- Let's find a permutation σ that maps a to 2 and b to 3. We can use the fact that if n ≥ 4, then 0, 1, 2, 3 are distinct in ZMod n.
      ...
      have h_cubic : represented_by_polynomial_of_degree (kronecker_delta n 2 3) 3 := by
        exact cubic_d_23 n;
      convert degree_perm_fix_01 n a b σ 3 hσ.2.2.1 hσ.2.2.2 _ using 1 ; aesop

theorem degree_d (n : ℕ) [NeZero n] (a b : ZMod n) :
  if a = b then represented_by_polynomial_of_degree (kronecker_delta n a b) 3
  else if ({a, b} : Finset (ZMod n)) = {0, 1} then represented_by_polynomial_of_degree (kronecker_delta n a b) 4
  else
    represented_by_polynomial_of_degree (kronecker_delta n a b) 3 := by
...
theorem quartic_d (n : ℕ) [NeZero n] (a b : ZMod n) :
  ...
  split_ifs at this <;> simp_all +decide [ represented_by_polynomial_of_degree ];
